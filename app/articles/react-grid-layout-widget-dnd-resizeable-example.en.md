...
date: "2025-02-17"
description: "This post demonstrates how to build a React grid layout with draggable and resizable widget components using the react-grid-layout library. We'll put together a simple dashboard where users can rearrange widgets however they like."
...
const useResize = (): UseResizeReturn => {
  ...
  // Calculate the height of each grid cell, accounting for gaps
...
// Header component for each widget with a drag handle
...
// Example widget content component that displays a large number
const BigNumber = () => (
...
// Initial set of available widgets
...
export default function App() {
  // Get responsive dimensions from the custom hook
  const { wrapperWidth, height } = useResize();

  // State to track widgets on the grid and the available widgets
  ...
  const handleOndrop = (
    ...
    // Find the dragged widget in the available widgets list
    const draggedWidget = availableWidgets.find((widget) => widget.id === id);
    if (draggedWidget === undefined) throw new Error("Widget not found");

    // Add the widget to the grid and remove it from the available widgets list
  ...
  return (
    <div className="gap-2 flex h-full w-full flex-1 bg-red-900">
      ...
      <div className="relative h-auto w-full bg-blue-900" id="reports">
        {/* Placeholder grid showing empty slots */}
